...
const Api = {
  ...
  createContent: (profileId, title, bodyText) => apiFetch('/contents', { method: 'POST', body: JSON.stringify({ profileId, title, bodyText }) }),
...
async function renderContenido() {
  ...
  async function load() {
    ...
    list.innerHTML = items.map((c) => `
      <div class="content-card">
        ...
        <div class="content-preview hidden" id="preview-${c.id}">${escapeHtml(c.bodyText)}</div>
      </div>
    `).join('');
  ...
  document.getElementById('btnAddContent').addEventListener('click', () => {
    ...
    document.getElementById('contentForm').addEventListener('submit', async (e) => {
      e.preventDefault();
      await Api.createContent(state.activeProfileId, document.getElementById('cTitle').value.trim(), document.getElementById('cBody').value);
